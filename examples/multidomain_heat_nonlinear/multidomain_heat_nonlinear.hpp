...
#ifndef LIFEX_EXAMPLE_MULTIDOMAIN_HEAT_NONLINEAR_HPP_
#define LIFEX_EXAMPLE_MULTIDOMAIN_HEAT_NONLINEAR_HPP_
...
{
  /**
   * @brief Class for multidomain solution of heat equation with a non-linear
   * reaction term.
   *
   * Solves the following boundary value problem:
   * @f[ \left\{\begin{alignedat}{2}
   *  \frac{\partial u}{\partial t} -D \Delta u + \mathbf\beta\cdot\nabla u - r
   * u^2 & = f & \quad &
   * \text{in}\;\Omega\times(0,T) \\
   *  D \nabla u \cdot \mathbf n & = 0 & \quad &
   * \text{on}\;\partial\Omega\times(0,T) \\ u(t = 0) & = 0 & \quad &
   * \text{in}\;\Omega \end{alignedat}\right. @f] where @f[ \Omega =
   * (-0.5, 1.5)\times(-0.5, 0.5)\times(-0.5, 0.5)\,. @f]
   *
   ...
   * @f[ \left\{ \begin{alignedat}{2}
   *  \frac{\partial u_0}{\partial t} -D \Delta u_0 + \mathbf\beta\cdot\nabla
   * u_0 - r u_0^2 & = f|_{\Omega_0} & \quad & \text{in}\;\Omega_0\times(0,T) \\
   ...
   * \text{on}\;\Sigma\times(0,T) \\ \frac{\partial u_1}{\partial t} - D \Delta
   * u_1 + \mathbf\beta\cdot\nabla u_1 - r u_1^2 & = f|_{\Omega_1} & \quad &
   ...
   * <br> The problem to be solved for each iteration is non-linear and solved
   * using Newton's method with the @ref utils::NonLinearSolverHandler class.
   ...
   * naturally, and no extra terms are needed. The system is non-linear, and
   * solved with Newton's method with the @ref utils::NonLinearSolverHandler class.
   */
  class ExampleMultidomainHeatNonLinear : public CoreModel
  {
  public:
    ...
    /// Constructor.
    ExampleMultidomainHeatNonLinear(
  ...
  protected:
    ...
    void
    assemble_monolithic(const bool &assemble_jac = true);
    ...
    std::array<std::unique_ptr<ExampleHeatNonLinear>, 2> subproblems;
    ...
    utils::NonLinearSolverHandler<LinAlg::MPI::BlockVector>
      non_linear_solver; ///< Non-linear solver handler.
    utils::LinearSolverHandler<LinAlg::MPI::BlockVector>
      linear_solver; ///< Linear solver handler.
    utils::BlockPreconditionerHandler
      preconditioner; ///< Preconditioner handler.
    ...
    unsigned int prm_bdf_order;    ///< BDF order for time discretization.
    double       prm_initial_time; ///< Initial time.
    double       prm_final_time;   ///< Final time.
...
#endif /* LIFEX_EXAMPLE_MULTIDOMAIN_HEAT_NONLINEAR_HPP_ */
