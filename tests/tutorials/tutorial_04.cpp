...
namespace lifex::tutorials
{
  ...
  /**
   * @brief Prototype solver class for a time-dependent non-linear PDE.
   *
   * This class also shows the use of @ref VerbosityParam
   * for handling parameter files verbosity.
   *
   ...
   * linearized and solved using Newton's method, @a i.e., given an initial
   * guess @f$u_0^{n+1}@f$ and for @f$k=1, \dots, n_\mathrm{max}@f$ until
   * convergence, the following iterative scheme is solved:
   * @f[
   * \begin{aligned}
   ...
   * \end{aligned}
   * @f]
   * where @f$u^n_{\mathrm{EXT}\sigma}@f$ is an extrapolation of
   * @f$u^{n+1}@f$, computed as a linear combination of the previous
   * time steps (see @ref utils::BDFHandler).
   *
   * @note To see @ref VerbosityParam in action, compare the use of
   * the <kbd>-g</kbd> flag to <kbd>-g minimal</kbd> and <kbd>-g full</kbd>
   * to generate parameter files, as explained in @ref run.
   */
  class Tutorial04 : public CoreModel
  {
  public:
    /// Constructor.
    Tutorial04()
                          ...
                          for (unsigned int j = 0; j < dofs_per_cell; ++j)
                            {
                              ...
                              cell_jac(i, j) +=
                                2 * u_ext_loc[q] * fe_values.shape_value(i, q) *
...
main(int argc, char **argv)
{
  ...
  try
    {
      lifex::tutorials::Tutorial04 tutorial;
